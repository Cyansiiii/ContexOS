...
function App() {
  ...
  const askQuestion = async () => {
    ...
    const timeout = setTimeout(() => controller.abort(new DOMException('Timeout', 'TimeoutError')), 300000)
    try {
      ...
      if (error.name === 'AbortError') {
        setAnswer('Request timed out. Ollama may be slow — try again.')
      } else {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      console.error(error);
      if (error.name === 'AbortError') {
        setUploadMessage('Timed out — Ollama took too long. Try a shorter text.');
      } else {
        setUploadMessage('Failed to upload memory. Is the backend running?');
      }
    } finally {
      clearTimeout(timeout)
      setUploading(false);
